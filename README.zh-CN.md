...
| **Retries** | 可重试失败(HTTP 5xx 或 429)后额外重发的次数,默认 **3**;填 0 表示不重试,上限 5。Test connection 固定只重试 1 次,不受该值影响 |
...
2. **批次**:去重后按 **8 条 / 400 字符**上限切批,**3 批并发**发送;单个文本节点为保持 DOM 边界可能超过 400 字符
...
- **缓存**:按「目标语言 + 模型 + 原文」为键(FNV-1a 64 位哈希),存于 `chrome.storage.local`;超出容量水位后按写入时间自动淘汰最早的条目
- **重试**:失败批次默认重试 **3 次**(设置页可调 0–5);HTTP 5xx 退避 1s / 2s / 4s,HTTP 429 退避 5s / 10s / 10s(单次等待封顶 10s);其他客户端错误、解析错误、超时与网络不可达直接失败
