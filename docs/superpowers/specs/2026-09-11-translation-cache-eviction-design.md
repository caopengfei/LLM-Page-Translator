...
- `getMany()` 的校验条件为 `rec.src === text && rec.lang === targetLang && typeof rec.dst === 'string'`（`lang` 校验防"键格式变更后旧记录串语言"静默命中，正常路径恒成立，失败即回源重译）。
- **键格式追加模型维度（相对初版设计的有意偏离）：** `tc:<目标语言>:<hash>` → `tc:<目标语言>:<encodeURIComponent(model)>:<hash>`。译文质量/风格随模型变化，换模型后旧缓存必须失配；若沿用初版无模型键，切换模型会静默命中旧模型的译文。旧的不带模型段的条目无法再被命中，成为孤儿，仅靠容量淘汰慢慢清掉（`at` 缺失视为最早，优先被淘汰），无需迁移脚本。
...
**逐条大小按 UTF-8 字节估算（实现细节）：** `entrySize` 用 `TextEncoder` 计算键与值序列化的字节数，而非 `String.length` 的 UTF-16 单元数。中文在存储中约 3 字节/字，按字符数会系统性低估 CJK 为主的缓存，导致淘汰提前停、后续每次写入都全量扫描。无 `TextEncoder` 的极端环境按"ASCII 1 字节、其余 2–3 字节"保守兜底（宁可高估多淘汰一点）。
